...
export function DashboardSidebar() {
  ...
  const [navigationCounts, setNavigationCounts] = useState<Record<string, number>>({})
    ...
    const fetchCounts = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setNavigationCounts(data)
        }
      } catch (error) {
        console.error("[Dashboard Sidebar] Error fetching navigation counts:", error)
      } finally {
    ...
    <div className="p-4">
      ...
      <nav className="space-y-1" aria-label="Main navigation">
        {navigationCategories.map((category) => {
          ...
          // Calculate total count using dynamic counts
          const totalCount = category.items.reduce((sum, item) => {
            const hrefKey = item.href.replace(/^\//, "").replace(/\//g, "-")
            const hrefKeyAlt = item.href.replace(/^\//, "")
            const dynamicCount = navigationCounts[item.href] ?? navigationCounts[hrefKey] ?? navigationCounts[hrefKeyAlt] ?? item.count ?? 0
            return sum + (countsLoading && item.count !== undefined ? (item.count || 0) : (dynamicCount || 0))
          }, 0)
          const hasAlerts = category.items.some((item) => {
            const hrefKey = item.href.replace(/^\//, "").replace(/\//g, "-")
            const hrefKeyAlt = item.href.replace(/^\//, "")
            const dynamicCount = navigationCounts[item.href] ?? navigationCounts[hrefKey] ?? navigationCounts[hrefKeyAlt] ?? item.count ?? 0
            const displayCount = countsLoading && item.count !== undefined ? item.count : dynamicCount
            return item.highlight === "alert" && displayCount && displayCount > 0
          })
              ...
              {isExpanded && (
                ...
                >
                  {category.items.map((item) => {
                    ...
                    // Get dynamic count - try multiple key formats
                    const hrefKey = item.href.replace(/^\//, "").replace(/\//g, "-")
                    const hrefKeyAlt = item.href.replace(/^\//, "")
                    const dynamicCount = navigationCounts[item.href] ?? navigationCounts[hrefKey] ?? navigationCounts[hrefKeyAlt] ?? item.count
                    const displayCount = countsLoading && item.count !== undefined ? item.count : (dynamicCount ?? item.count)
                    const isAlert = item.highlight === "alert" && displayCount && displayCount > 0
