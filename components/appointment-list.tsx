...
export function AppointmentList({
  ...
  const filterAppointments = (appointments: AppointmentData[]) => {
    ...
    return appointments.filter(
      (apt) =>
        ...
        `${apt.providers.first_name} ${apt.providers.last_name}`.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const AppointmentCard = ({ appointment, showDate = false }: { appointment: AppointmentData; showDate?: boolean }) => (
    <div className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
      <div className="flex items-center space-x-4">
        <div className="text-center">
          {showDate && (
            <div className="font-semibold text-sm">{new Date(appointment.appointment_date).toLocaleDateString()}</div>
          )}
          <div className="font-semibold text-sm">{formatTime(appointment.appointment_time)}</div>
        ...
        <div>
          ...
          <div className="flex items-center space-x-2 text-sm text-muted-foreground">
            ...
            {appointment.providers.title && <span>, {appointment.providers.title}</span>}
            <span>•</span>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex items-center gap-1">
          ...
          {appointment.providers.id === currentProviderId && (
            <>
              <EditAppointmentDialog appointment={appointment} patients={patients} providers={providers}>
                <Button variant="ghost" size="sm">
          ...
          <CardContent>
            <div className="space-y-4">
              {filterAppointments(todayAppointments).length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">No appointments found for today.</div>
              ) : (
          ...
          <CardContent>
            <div className="space-y-4">
              {filterAppointments(upcomingAppointments).length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">No upcoming appointments found.</div>
              ) : (
      ...
      <TabsContent value="past">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Previous appointments and sessions</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
