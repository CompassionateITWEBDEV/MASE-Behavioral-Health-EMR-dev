...
export default function CaseloadTransferPage() {
  ...
  async function fetchStaff() {
    try {
      console.log("[Caseload Transfer] Fetching staff...");
      const response = await fetch("/api/staff?active=true");
      
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        console.error("[Caseload Transfer] API error:", errorData);
      ...
      console.log("[Caseload Transfer] Loaded staff:", {
      ...
      console.error("[Caseload Transfer] Error fetching staff:", error);
      toast({
        title: "Error",
                ...
                <div className="grid md:grid-cols-3 gap-4 items-end">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={toStaffId} onValueChange={setToStaffId}>
                      ...
                      <SelectContent>
                        {staff.filter((s) => s.id !== fromStaffId).length === 0 ? (
                          <div className="px-2 py-6 text-center text-sm text-muted-foreground">
                            {fromStaffId ? "No other staff members available" : "Select 'From' staff member first"}
                          </div>
                        ) : (
                          staff
