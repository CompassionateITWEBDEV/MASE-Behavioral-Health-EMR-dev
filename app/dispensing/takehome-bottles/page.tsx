...
export default function TakeHomeBottlesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Fetch patient data separately for all admissions
      const patientIds = (admissions || []).map((a: any) => a.patient_id).filter(Boolean)
      let patientsData: any[] = []
      
      if (patientIds.length > 0) {
        const { data, error: patientsDataError } = await supabase
          .from("patients")
          .select("id, first_name, last_name, date_of_birth")
          .in("id", patientIds)

        if (patientsDataError) {
          console.error("Error fetching patients data:", patientsDataError)
        } else {
          patientsData = data || []
        }
      }
      ...
      const patientDataMap = new Map(patientsData.map((p: any) => [p.id, p]))
      ...
      // Build patients for dispensing list - deduplicate by patient_id
      const patientsForDispensingMap = new Map<string, PatientForDispensing>()
      
      ;(admissions || []).forEach((admission: any) => {
        const patientId = admission.patient_id
        // Keep only the first admission per patient to avoid duplicates
        if (!patientsForDispensingMap.has(patientId)) {
          ...
          patientsForDispensingMap.set(patientId, {
      ...
      const patientsWithStatus: PatientForDispensing[] = Array.from(patientsForDispensingMap.values())
