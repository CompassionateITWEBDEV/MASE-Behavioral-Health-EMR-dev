...
export default function Form222Page() {
  ...
  const stats = {
    ...
    pendingReceipts: forms.reduce(
      (sum, f) =>
        sum + (f.lines?.filter((l) => l.status === "pending").length || 0),
      0
    ),
    expiringSoon: forms.filter((f) => {
      if (f.status !== "executed") return false;
      const daysLeft = Math.ceil(
        (new Date(f.expires_at).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
      );
  ...
  const handleExecuteForm = async () => {
    ...
    if (newFormLines.some((l) => !l.medication || l.quantity <= 0)) {
      toast({
        title: "Validation Error",
        description:
          "Please complete all line items with medication and quantity",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleProcessReceiving = async () => {
    ...
    try {
      ...
      if (form && form.lines && form.lines.length > 0) {
        const allComplete = form.lines.every((l: Form222Line) => {
          const receivingLine = receivingLines.find((rl) => rl.lineId === l.id);
          return (
            (receivingLine?.containersReceived ?? 0) > 0 ||
            l.status === "complete"
          );
        });
        if (allComplete) {
  ...
  const handleAddPoa = async () => {
    if (
      !newPoa.authorizedName ||
      !newPoa.registrantDea ||
      !newPoa.expirationDate
    ) {
      toast({
        title: "Validation Error",
        description: "Please fill in all required fields",
  ...
  const addFormLine = () => {
    setNewFormLines([
      ...newFormLines,
      {
        lineNumber: newFormLines.length + 1,
        medication: "",
        quantity: 0,
        unit: "mL",
      },
      ...
      <div className="flex-1 ml-64 p-6 space-y-6">
        ...
        <Alert className="bg-red-50 border-red-200">
          ...
          <AlertDescription className="text-red-700">
            <strong>DEA Form 222 - Schedule II Ordering:</strong> Only DEA
            registrants or authorized POA signers may execute Form 222. No
            alterations permitted - changes void the form. 60-day expiry from
            execution date.
          </AlertDescription>
        </Alert>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Schedule II controlled substance ordering and receiving
            </p>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {stats.authorizedSigners}
              </div>
              <p className="text-xs text-muted-foreground">
                Registrant + {poaList.length} POA
              </p>
            </CardContent>
          </Card>
        </div>
          ...
          <TabsContent value="register" className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  All executed forms with 60-day tracking and receiving status
                </CardDescription>
              </CardHeader>
              <CardContent>
                {loading ? (
                        ...
                        <div key={form.id} className="border rounded-lg p-4">
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm text-muted-foreground mb-3">
                            <div className="flex items-center">
                              ...
                              {form.supplier_name} (DEA:{" "}
                              {form.supplier_dea_number})
                            </div>
                            <div className="flex items-center">
                            ...
                            <div className="space-y-2">
                              ...
                              {form.lines.map((line) => (
                                <div
                                  ...
                                  <div className="flex items-center space-x-2">
                                    <Badge
                                      className={getLineStatusColor(
                                        line.status
                                      )}>
                                      {line.status}
                                    </Badge>
                                    <span className="text-xs text-muted-foreground">
                                      {line.containers_received}/
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Manage authorized signers for Form 222 execution
                  </CardDescription>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {poaList.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">
                        Add authorized signers to delegate Form 222 execution
                      </p>
                    </div>
                  ) : (
                    poaList.map((poa) => (
                      <div key={poa.id} className="p-4 border rounded-lg">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                              DEA: {poa.registrant_dea_number} • Effective:{" "}
                              {poa.effective_date} to {poa.expiration_date}
                            </p>
                          </div>
                          <Badge
                            className={
          ...
          <TabsContent value="reports" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Generate reports for DEA inspection and audit compliance
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      toast({
                        ...
                        description:
                          "Form 222 Register report is ready for download",
                      });
                    }}>
                    <FileText className="w-6 h-6" />
                    <span>Form 222 Register</span>
                    <span className="text-xs text-muted-foreground">
                      All forms with line items
                    </span>
                  </Button>
                  <Button
                    variant="outline"
                    ...
                    onClick={() => {
                      toast({
                        title: "Report Generated",
                        description:
                          "Receiving Log report is ready for download",
                      });
                    }}>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      toast({
                        title: "Report Generated",
                        description:
                          "Voided Forms report is ready for download",
                      });
                    }}>
                    <XCircle className="w-6 h-6" />
                    <span>Voided Forms</span>
                    <span className="text-xs text-muted-foreground">
                      Forms voided with reasons
                    </span>
                  </Button>
                  <Button
                    variant="outline"
                    ...
                    onClick={() => {
                      toast({
                        title: "Report Generated",
                        description:
                          "POA Authorization report is ready for download",
                      });
                    }}>
        ...
        <Dialog open={showNewForm} onOpenChange={setShowNewForm}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Order Schedule II controlled substances - one supplier, one item
                per line
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-6">
              <Alert>
                ...
                <AlertDescription>
                  <strong>DEA Requirements:</strong> Only registrant or
                  POA-authorized personnel may sign. Form expires 60 days from
                  execution. No alterations permitted.
                </AlertDescription>
              </Alert>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewForm({ ...newForm, supplierName: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewForm({ ...newForm, supplierDea: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setNewForm({ ...newForm, supplierAddress: e.target.value })
                  }
                />
              </div>
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setNewForm({ ...newForm, signerId: value })
                      }>
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        <SelectItem value="1">
                          DEA Registrant (Primary)
                        </SelectItem>
                        {poaList.map((poa) => (
                          <SelectItem
                            key={poa.id}
                            value={String(poa.authorized_user_id || poa.id)}>
                ...
                <div className="mt-3 p-3 bg-white border rounded">
                  <p className="text-sm text-muted-foreground">
                    ...
                    By executing this form, I certify that I am authorized to
        ...
        <Dialog open={showReceiving} onOpenChange={setShowReceiving}>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Record containers received and arrival dates
              </DialogDescription>
            </DialogHeader>
                      ...
                      <div className="flex items-center space-x-3">
                        <div>
                          ...
                          <Input
                            ...
                            type="number"
                            value={
                              receivingLines[index]?.containersReceived || 0
                            }
                            onChange={(e) => {
        ...
        <Dialog open={showViewForm} onOpenChange={setShowViewForm}>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              <DialogTitle>
                Form 222 Details - {selectedForm?.form_number}
              </DialogTitle>
            </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {selectedForm.supplier_dea_number}
                    </p>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">
                  ...
                  <div className="space-y-2 mt-2">
                    {selectedForm.lines?.map((line) => (
                      <div
                        ...
                        <span>
                          #{line.line_number} - {line.quantity_ordered}{" "}
                          {line.unit}
                        </span>
        ...
        <Dialog open={showVoidDialog} onOpenChange={setShowVoidDialog}>
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button
                variant="destructive"
                onClick={handleVoidForm}
                disabled={saving || !voidReason}>
        ...
        <Dialog open={showPoaDialog} onOpenChange={setShowPoaDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Authorize personnel to execute Form 222 on behalf of the
                registrant
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewPoa({ ...newPoa, authorizedName: e.target.value })
                  }
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewPoa({ ...newPoa, registrantDea: e.target.value })
                  }
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewPoa({ ...newPoa, effectiveDate: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewPoa({ ...newPoa, expirationDate: e.target.value })
                    }
                  />
                </div>
              </div>
