          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {selectedStage === "all" ? "All Intake Patients" : getStageInfo(selectedStage).name}
                <Badge variant="secondary">
                  {selectedStage === "all" ? filteredPatients.length : getStagePatients(selectedStage).length} patients
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {(selectedStage === "all" ? filteredPatients : getStagePatients(selectedStage)).map((patient) => {
                ...
                {(selectedStage === "all" ? filteredPatients : getStagePatients(selectedStage)).length === 0 && (
                  <div className="text-center py-8">
                    <Users className="mx-auto h-12 w-12 text-muted-foreground mb-4" />
                    <h3 className="text-lg font-medium mb-2">No patients in queue</h3>
                    <p className="text-muted-foreground">
                      Patients will appear here when they enter the intake process
                    </p>
                  </div>
                )}
              </div>
