...
export default function IntakeQueuePage() {
  ...
  const handleMoveToNextStage = async (patientIdOrDisplayId: string) => {
    // Find patient by either patientId or display id (INT-xxx)
    const patient = intakePatients.find(
      (p) => p.patientId === patientIdOrDisplayId || p.id === patientIdOrDisplayId
    );
    ...
    // Use the actual patientId (UUID) from the patient object
    const actualPatientId = patient.patientId;
    
    if (!actualPatientId) {
      toast({
        title: "Error",
        description: "Patient ID not available",
        variant: "destructive",
      });
      return;
    }

    setMovingPatient(actualPatientId);
    try {
      const response = await fetch(`/api/intake/patients/${actualPatientId}/stage`, {
        method: "PATCH",
        headers: {
      ...
      toast({
        title: "Success",
        description: `Patient moved to ${getStageInfo(data.stage)?.name || data.stage}`,
      });

      // Refresh the queue to show updated stage
      await loadIntakePatients();
    } catch (err) {
      console.error("[Intake Queue] Error moving patient:", err);
      toast({
        title: "Error",
        description: err instanceof Error ? err.message : "Failed to move patient to next stage",
        variant: "destructive",
      });
    } finally {
  ...
  const handleCollectUDS = (patientIdOrDisplayId: string) => {
    const patient = intakePatients.find(
      (p) => p.patientId === patientIdOrDisplayId || p.id === patientIdOrDisplayId
    );
    if (patient) {
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          {patient.currentStage === "collector-queue" && (
                            <Button
                              size="sm"
                              onClick={() => handleCollectUDS(patient.patientId || patient.id)}
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleMoveToNextStage(patient.patientId || patient.id)}
                            disabled={movingPatient === patient.patientId || patient.currentStage === "dosing"}
                            title={patient.currentStage === "dosing" ? "Patient is at final stage" : "Move to next stage"}>
                            {movingPatient === patient.patientId ? (
