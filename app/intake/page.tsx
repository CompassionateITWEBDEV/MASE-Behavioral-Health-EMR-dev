...
export default function PatientIntake() {
  ...
  const [consentFormsData, setConsentFormsData] = useState<any>(null)
    ...
    try {
      const response = await fetch("/api/intake/progress", {
        ...
        body: JSON.stringify({
          ...
          consent_forms_data: consentFormsData,
        }),
      })
  ...
  // Handle consent forms completion
  const handleConsentFormsComplete = async (data: any) => {
    if (!selectedPatient) return

    setConsentFormsData(data)
    
    // Update documentation status based on completed forms
    const updatedStatus = { ...documentationStatus }
    
    // Map consent form IDs to documentation status keys
    const formMapping: Record<string, keyof typeof documentationStatus> = {
      "consent-to-treatment": "consent_for_treatment",
      "hipaa-privacy-notice": "hipaa_authorization",
      "release-of-information": "emergency_contact_form",
    }

    // Update status for completed required forms
    Object.entries(data.consentForms || {}).forEach(([formId, formData]: [string, any]) => {
      if (formData?.completed && formMapping[formId]) {
        updatedStatus[formMapping[formId]] = "completed"
      }
    })
  ...
  const loadSavedProgress = async (patientId: string) => {
    try {
      ...
      if (data.progress) {
        ...
        // Restore consent forms data
        if (progress.consent_forms_data) {
          setConsentFormsData(progress.consent_forms_data)
        }
    ...
    <div className="flex min-h-screen bg-background">
                  ...
                  <TabsContent value="documentation">
                    <Card>
                      ...
                      <CardContent className="space-y-4">
                          ...
                          <div className="flex items-center justify-between mb-2">
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground mt-1">
                                Complete all required consent and authorization forms
                              </p>
                            </div>
                            <Button
                              onClick={() => setShowConsentForms(true)}
                              disabled={!selectedPatient}
                            >
                              <FileText className="mr-2 h-4 w-4" />
                              {consentFormsData ? "Review/Edit Forms" : "Start Consent Forms"}
                            </Button>
                          </div>
                          {consentFormsData && (
                            <div className="mt-3 p-3 bg-background rounded border">
                              <div className="flex items-center justify-between text-sm">
                                ...
                                <Badge variant="default">
                                  {consentFormsData.completionStats?.requiredCompleted || 0} / {consentFormsData.completionStats?.totalRequired || 0} Required
                                </Badge>
                              </div>
                              <div className="mt-2">
                                <Progress 
                                  value={
                                    consentFormsData.completionStats?.totalRequired 
                                      ? (consentFormsData.completionStats.requiredCompleted / consentFormsData.completionStats.totalRequired) * 100 
                                      : 0
                                  } 
                                  className="h-2"
                                />
                              </div>
                              {consentFormsData.completedAt && (
                                <p className="text-xs text-muted-foreground mt-2">
                                  Last updated: {new Date(consentFormsData.completedAt).toLocaleString()}
                                </p>
                              )}
                            </div>
                          )}
                        ...
                        <div className="space-y-3">
                          <h4 className="font-medium text-sm">Other Documentation</h4>
      ...
      {selectedPatient && (
        <ComprehensiveConsentForms
          ...
          isOpen={showConsentForms}
          onClose={() => setShowConsentForms(false)}
          onComplete={handleConsentFormsComplete}
        />
      )}
    </div>
  )
