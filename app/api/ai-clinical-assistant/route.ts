...
export async function POST(request: Request) {
  try {
    ...
    const analysis = {
      clinicalSummary: generateClinicalSummary(
        patient,
        medications ?? [],
        vitals ?? [],
        labs ?? []
      ),
      riskAlerts: identifyRiskAlerts(
        patient,
        medications ?? [],
        vitals ?? [],
        labs ?? []
...
function generateClinicalSummary(
  ...
  return {
    ...
    keyFindings: [
      ...
      "Hypertension trending up",
    ],
  };
}

function identifyRiskAlerts(
  patient: any,
  medications: any[],
  vitals: any[],
  labs: any[]
  ...
  return [
    {
      level: "critical",
      message: "Declining renal function with current metformin dose",
    },
    {
      level: "warning",
      message: "Blood pressure trending up over last 3 visits",
    },
  ...
  return [
    {
      ...
      recommendation: "Consider adding GLP-1 agonist",
      evidence: "SUSTAIN-6 trial",
    },
    {
      category: "Hypertension",
      recommendation: "Increase lisinopril dose",
      evidence: "ADA/ACC guidelines",
    },
    {
...
function generateDifferentialDx(
  patient: any,
  encounters: any[],
  vitals: any[]
) {
  return [
    ...
    {
      diagnosis: "Uncontrolled Diabetes with Complications",
      probability: "High",
      confirmed: false,
    },
  ];
