...
export async function GET(request: NextRequest) {
  try {
    ...
    const patientIds = (prescriptions || []).map((rx) => rx.patient_id).filter(Boolean)
    let transferMap = new Map()
    
    if (patientIds.length > 0) {
      const { data: transfers } = await supabase
        .from("patients")
        .select("id, is_transferred, transferred_to, transferred_at")
        .in("id", patientIds)
        ...
        transfers.forEach((p) => {
          if (p.is_transferred) {
            transferMap.set(p.id, {
              is_transferred: true,
              transferred_to: p.transferred_to,
    ...
    const formattedPrescriptions = (prescriptions || []).map((rx) => {
      const transferInfo = transferMap.get(rx.patient_id)
      return {
        ...rx,
        ...
        patient_transferred: transferInfo?.is_transferred || false,
        transferred_to: transferInfo?.transferred_to || null,
        transferred_at: transferInfo?.transferred_at || null,
      }
    })
