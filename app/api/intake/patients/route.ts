...
function determineIntakeStage(admission: any, progressData: any): string {
  // Check for manual stage override first (highest priority)
  if (progressData?.manual_stage) {
    return progressData.manual_stage;
  }
  if (progressData?.current_stage) {
    return progressData.current_stage;
  }
  ...
  if (admission.status === "active") {
    if (progressData) {
      ...
      // Check if UDS is completed
      const assessmentData = progressData.assessment_data || {};
      const udsCompleted = progressData.uds_completed || assessmentData.uds_completed;
      
      if (!udsCompleted) {
        return "collector-queue";
      }

      // UDS done, check nurse assessment
      const nurseAssessmentCompleted = progressData.nurse_assessment_completed || 
        assessmentData.nurse_assessment_completed;
      
      if (!nurseAssessmentCompleted) {
        return "nurse-queue";
      }

      // Nurse assessment done, check counselor evaluation
      const counselorEvaluationCompleted = progressData.counselor_evaluation_completed || 
        assessmentData.counselor_evaluation_completed;
      
      if (!counselorEvaluationCompleted) {
        return "counselor-queue";
      }

      // Counselor evaluation done, check doctor approval
      const doctorApprovalCompleted = progressData.doctor_approval_completed || 
        assessmentData.doctor_approval_completed;
      
      if (!doctorApprovalCompleted) {
        return "doctor-queue";
      }

      // All assessments and approvals done, ready for dosing
      return "dosing";
    }
