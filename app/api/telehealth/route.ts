  ...
  try {
    ...
    const { data: appointments, error: appointmentsError } = await supabase
      ...
      .select(
        ...
        providers(id, first_name, last_name, role)
      `
      )
    ...
    const formattedAppointments = (appointments || []).map((apt: any) => {
      // Handle patients - could be array or single object
      const patient = Array.isArray(apt.patients)
        ? apt.patients[0]
        : apt.patients;
      // Handle providers - could be array or single object
      const provider = Array.isArray(apt.providers)
        ? apt.providers[0]
        : apt.providers;
    ...
    return NextResponse.json(
      { error: "Failed to fetch telehealth data" },
      { status: 500 }
    );
  }
}
